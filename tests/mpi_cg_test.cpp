...
class mpi_cg_test: public testing::Test
{
    protected:
...
TEST_F(mpi_cg_test, cg1)
{
    ...
    // ======================================================== //
    timer.start();
    yakutat::mpi::cg<yakutat::mpi::SparseMatrixELL<double, 4>> solver;
...
TEST_F(mpi_cg_test, cg2)
{
    stopWatch timer;
    size_t n = 4;
    mpi_tools world;
    world.MPI_division(n, myid, nproc);
    size_t max_row_nnz = n;
    yakutat::mpi::SparseMatrixELL<double, 4> lhs_mat(n);
    std::vector<double> rhs(n, 0.0);
    std::vector<double> x(n, 0.0);
    size_t iters;
...
TEST_F(mpi_cg_test, cg_DynamicMatrix)
{
    stopWatch timer;
    size_t n = 4;
    mpi_tools world;
    ...
    // ======================================================== //
    timer.start();
    yakutat::mpi::cg<yakutat::mpi::DynamicMatrix<double>> solver;
    EXPECT_TRUE(solver.initialize(lhs_mat));
    EXPECT_TRUE(solver.partition(world));
    EXPECT_TRUE(solver.setTolerance(1e-20));
    std::tie(iters, error) = solver.solve(rhs, x);
