...
class bicgstab_test: public testing::Test
{
    protected:
...
TEST_F(bicgstab_test, bicgstab1)
{
    stopWatch timer;
    size_t n = 4;
    ...
    // ======================================================== //
    timer.start();
    yakutat::bicgstab<yakutat::SparseMatrixELL<double, 4>> solver;
    ...
    std::cout << "bicgstab elapsedTime : " << timer.elapsedTime() << std::endl;
}


TEST_F(bicgstab_test, bicgstab2)
{
    stopWatch timer;
    size_t n = 4;
    yakutat::SparseMatrixELL<double, 4> lhs_mat(n);
...
TEST_F(bicgstab_test, bicgstab_DynamicMatrix)
{
    stopWatch timer;
    size_t n = 4;
    ...
    // ======================================================== //
    timer.start();
    yakutat::bicgstab<yakutat::DynamicMatrix<double>> solver;
    EXPECT_TRUE(solver.initialize(lhs_mat));
    EXPECT_TRUE(solver.setTolerance(1e-20));
    std::tie(iters, error) = solver.solve(rhs, x);
    timer.stop();
